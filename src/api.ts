import type { AuthResponse, Expense, Group, GroupSummary, Member, Message, Settlement, User } from "./types";
...
export const api = {
  ...
  createGroup: (name: string, members: string[]) => request<Group>("/groups", { method: "POST", body: JSON.stringify({ name, members }) }),
  group: (id: string) => request<Group>(`/groups/${id}`),
  addMember: (id: string, name: string) => request<Member>(`/groups/${id}/members`, { method: "POST", body: JSON.stringify({ name }) }),
